...
重要：用户问题中的邮件主题和正文内容部分必须用中文双引号“”包裹，例如：帮我给张三发封邮件，主题写“项目进度”，内容是“请在周五前提交报告”。"""
    },
...
重要：用户问题中的短信内容部分必须用中文双引号“”包裹，例如：帮我给小李发条短信，说“我大概十分钟后到”。"""
    },
    ...
    "toggle_wifi": {
        ...
        "param_notes": "enable（BOOLEAN，可选）：true 打开，false 关闭。"
    },
...
同时出现时以 level 为准。""" 
    ...
    "set_mute": {
        ...
        "param_notes": "enable（BOOLEAN，可选）：true 静音，false 取消静音。"
    },
    ...
    "toggle_dnd": {
        ...
        "param_notes": "enable（BOOLEAN，可选）：true 开启，false 关闭。"
    },
...
datetime（STRING，可选）：日期时间 YYYY-MM-DDTHH:MM:SS。不提供则查今天。""" 
    ...
    "create_calendar_event": {
        "system": """你是 function call 训练数据生成助手。用户想在自己的日历中创建一个日程事件。

关键要求：用户问题必须明确表达"在日历/日程中创建或添加事件"的意图，而不是下达"安排/组织某个活动"的指令。
...
错误示例（像在给秘书下指令，而不是操作日历）：
- "安排明天下午的会议"  ← 太像下达指令
- "约后天上午的牙医"    ← 像在叫人去预约
- "组织下周三的团建"    ← 像在分配任务

表达多样化，覆盖工作、生活、学习、医疗、社交等场景。""",
...
start_date（STRING，可选）：查询起始日期时间 YYYY-MM-DDTHH:MM:SS。
end_date（STRING，可选）：查询结束日期时间 YYYY-MM-DDTHH:MM:SS。""" 
    },
...
重要：用户问题中的笔记标题和正文内容部分必须用中文双引号“”包裹，例如：帮我记一条笔记，标题是“周末计划”，内容写“周六爬山，周日看电影”。"""
    },
