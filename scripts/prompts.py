...
TOOL_PROMPTS = {
    ...
    "set_brightness": {
        ...
        "param_notes": """level 和 direction 二选一，不能同时出现，不能都缺。
- level（STRING，可选）：目标亮度值，如 \"80\"、\"30\"。用于用户明确说数值时。
- direction（STRING，可选）：\"high\"（调高）或 \"low\"（调低）。用于用户说\"调亮/调暗一点\"时。""",
        "bad_examples": "不要 level 和 direction 同时出现。用户说\"调到80\"则只给 level，说\"调亮\"则只给 direction:\"high\"。",
    },
...
repeat（STRING，可选）：重复模式，如\"daily\"\"weekly\"\"weekdays\"。""",
    ...
    "set_volume": {
        ...
        "param_notes": """level 和 direction 二选一，不能同时出现，不能都缺。
- level（STRING，可选）：目标音量值，如 \"80\"、\"30\"。用于用户明确说数值时。
- direction（STRING，可选）：\"high\"（调高）或 \"low\"（调低）。用于用户说\"调大/调小一点\"时。""",
        "bad_examples": "不要 level 和 direction 同时出现。不要说\"静音\"（那是set_mute）。",
    },
